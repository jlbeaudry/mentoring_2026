...
header-includes:
  ...
  - \renewcommand{\headrulewidth}{0pt} # removes the line frofm the header
...
options(knitr.duplicate.label = "allow", # use this when you use the child files
        knitr.kable.format = "latex") # hopefully I can use this with my tables
```
...
names_long <- df %>% 
  select (-c(`first_name`:`name and college`, preferred_name:title, work_phone:contract_end_notes, why_mentor:mentoring_approach)) %>% 
  pivot_longer(
    cols = planning:other_1, #did not include 'other_2' because it was empty
    names_to = "areas", 
    values_to = "rank"  
               )
...
names <- names_long2 %>% 
  select (c(id, name, college, level_recode, areas, rank, interest, role)) %>% 
  filter(interest == "yes")
