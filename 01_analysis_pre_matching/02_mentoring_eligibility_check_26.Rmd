...
df_tor <- df_tor %>% 
  mutate (level_fixed = recode (as.character(id),
                                "mentor_28" = "Level D", 
                            .default = as.character(level))) %>% 
  mutate(level_fixed = as.factor(level_fixed))
...
df_tee <- df_tee %>%
  mutate(previous_mentee_eligibility = case_when(id %in% "mentee_7" ~ "Not eligible", # participated in 2020
                                           TRUE ~ as.character("Eligible"))) %>% 
...
df_tee_end <- df_tee %>% 
  filter(str_detect(employment_length, "Fixed-term")) %>% 
  select(c(id, name, college, employment_length, contract_end_month, contract_end_year, level, employment_eligibility, level_eligibility, conferral_eligibility))
...
areas <- long2 %>% 
  select (c(id,`name and college`, name, college, level, areas, rank, interest, role)) %>% 
  filter(interest == "yes")
...
col_tor_details <- df_tor %>% 
  select (c(college, name, level, id)) %>% 
