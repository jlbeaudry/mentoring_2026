...
# [note: this is determined in the 00_.R file. We just need info for the report.
...
```{r withdrawals and errors, include = FALSE, eval = TRUE}

# Withdrawals and errors

## MENTEES

# 1 withdrawals this year [1 after matching]

withdraw_n_tee_pre <- df_tee %>%
    filter(id %in% c("mentee_40")) %>%
    nrow()
# 
...
df_tor <- df_tor %>%
  mutate(contract_end_notes = case_when(id %in% "mentor_13" ~ "Converted to continuing", #email confirmation
                                        ...
                                        id %in% "mentor_35" ~ "Not eligible", # via email
         TRUE ~ as.character(""))) %>% 
      relocate(contract_end_notes, .after = contract_end_year)

df_tor <- df_tor %>%
  mutate(contract_eligibility = case_when(str_detect(contract_end_notes, "Not eligible") ~ "Not eligible", 
...
df_tee <- df_tee %>%
  mutate(previous_mentee_eligibility = case_when(id %in% "mentee_7" ~ "Eligible", # participated in 2020, which was disrupted by COVID
                                           TRUE ~ as.character("Eligible"))) %>% 
...
df_tee <- df_tee %>%
  mutate(employment_eligibility = case_when(id %in% "mentee_17" ~ "Eligible", # completed the form
                                            id %in% "mentee_35" ~ "Not eligible", # Research Assistant
                                            id %in% "mentee_25" ~ "Not eligible", # Research Assistant
                                            TRUE ~ as.character("Eligible"))) %>% 
  mutate(employment_eligibility = as.factor(employment_eligibility))
...
df_tee <- df_tee %>%
  mutate(conferral_eligibility = case_when(id %in% "mentee_23" ~ "Eligible", # via email
                                  id %in% "mentee_25" ~ "Not eligible", # Just submitted & no contract extension via email
                                  id %in% "mentee_35" ~ "Not eligible", # no PhD
                                  TRUE ~ as.character("Eligible"))) %>% 
...
df_tee <- df_tee %>%
  mutate(contract_eligibility = case_when(id %in% "mentee_28" ~ "Not eligible", # via email
                                 id %in% "mentee_34" ~ "Eligible", # via email
                                 id %in% "mentee_2" ~ "Eligible", # via email 
                                 id %in% "mentee_5" ~  "Eligible", # via email 
                                 id %in% "mentee_6" ~ "Not eligible", # via email 
                                 id %in% "mentee_24" ~ "Eligible", # via email 
                                 id %in% "mentee_27" ~ "Not eligible", # via email
                                 id %in% "mentee_30" ~ "Eligible", # via email 
                            #    id %in% "mentee_40" ~ "Not eligible", # via email, but already removed as withdrawal
...
df_tee_end_26 <- df_tee %>% 
  filter(str_detect(employment_length, "Fixed-term")) %>% 
  select(c(id, name, college, employment_length, contract_end_year, level_recode, employment_eligibility, level_eligibility, conferral_eligibility)) %>% 
  filter(grepl("25", contract_end_year))
...
eligibility criteria for the program. Below are the screening steps:
  ...
  *  Withdrawals: We had `r withdraw_n_tee_pre` mentee withdraw before being matched with 
  a mentor. 
  
  *  Mentor eligibility:  We had `r nrow(df_tor_inel)` mentors who did not meet the 
  ...
  *  Mentee eligibility: We had
  ...
  had `r employ_no_n_tee` applications from people who did not hold academic positions and
  `r contract_no_n_tee` applications from people whose contracts will end before November 2026. 
  For those mentees from Colleges with a College-based mentoring program, I connected 
  the ineligible ECRs with people in their Colleges who could discuss 
  alternate options

  *  Missing information: We removed no mentees or mentors for providing incomplete
  information about the areas for growth.
...
# same re the report, but includes mentee number
